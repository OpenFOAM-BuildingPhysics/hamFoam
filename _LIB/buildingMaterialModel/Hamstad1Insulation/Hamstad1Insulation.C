...
{
    defineTypeNameAndDebug(Hamstad1Insulation, 0);
...
Foam::buildingMaterialModels::Hamstad1Insulation::Hamstad1Insulation
(
    const word& name,
...
void Foam::buildingMaterialModels::Hamstad1Insulation::update_w_C_cell(const volScalarField& pc, volScalarField& w, volScalarField& Crel, label& celli)
{
    List<scalar> reta; reta.setSize(1); reta[0]=-2.0e-4;
    List<scalar> retn; retn.setSize(1); retn[0]=2.0e0;
    List<scalar> retm; retm.setSize(1); retm[0]=0.5e0;
    ...
    w.ref()[celli] = w_tmp*900;   
    Crel.ref()[celli] = mag( C_tmp*900 );   
}

//- Correct the buildingMaterial liquid permeability (cell)
void Foam::buildingMaterialModels::Hamstad1Insulation::update_Krel_cell(const volScalarField& pc, const volScalarField& w, volScalarField& Krel, label& celli)
{
...
void Foam::buildingMaterialModels::Hamstad1Insulation::update_Kvap_cell(const volScalarField& pc, const volScalarField& w, const volScalarField& T, volScalarField& K_v, volScalarField& K_pt, label& celli)
{
    scalar rho_l = 1.0e3; 
    ...
    scalar tmp = 1 - (w.internalField()[celli]/900); 
    scalar delta = 2.61e-5 * tmp/(R_v*T.internalField()[celli]*9.6*(0.503*tmp*tmp + 0.497)); // Water vapour diffusion coefficient "for Insulation" [s]
