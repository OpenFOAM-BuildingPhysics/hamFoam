...
void Foam::HAMexternalMoistureFluxFvPatchScalarField::updateCoeffs()
{
    ...
    interpolationTable<scalar> RHair
    (
        "$FOAM_CASE/0/RHair"
    ); 
    ...
    scalar pvsat_s = exp(6.58094e1-7.06627e3/Tambient(time.value())-5.976*log(Tambient(time.value())));
    scalarField pv_s = pvsat_s*exp((pcp)/(rhol*Rv*Tambient(time.value())));

    scalar pvsat_o = exp(6.58094e1-7.06627e3/Tambient(time.value())-5.976*log(Tambient(time.value())));
    scalar pv_o = pvsat_o*RHair(time.value());

    scalarField g_conv = beta(time.value())*(pv_o-pv_s);
